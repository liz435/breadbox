...
// can snapshot `process.env.ANTHROPIC_API_KEY` — is to load this file
// at the top of the entry point. A prompt-injected agent that later
// reads `process.env` sees the slot already deleted.

import { captureAnthropicApiKey } from "./secrets"

const raw = process.env.ANTHROPIC_API_KEY ?? ""
captureAnthropicApiKey(raw)
