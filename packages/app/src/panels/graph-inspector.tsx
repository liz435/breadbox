...
const selectClass =
  "text-xs text-foreground bg-card border border-border rounded px-1.5 py-0.5 outline-none cursor-pointer hover:border-border transition-colors w-full";
const numInputClass =
  "bg-card border border-border rounded px-2 py-1 text-xs outline-none w-full transition-colors focus:border-border text-foreground";
const checkboxClass =
  "w-3.5 h-3.5 rounded border border-border bg-card accent-blue-500 cursor-pointer";
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex flex-wrap gap-1">
        {keys.map((key) => (
          <span
            key={key}
            className="inline-flex items-center gap-1 bg-secondary border border-border rounded px-1.5 py-0.5 text-[10px] text-foreground font-mono"
          >
            {key}
            <button
  ...
  return (
    <div className="flex flex-col gap-2">
      {actions.map((action, idx) => (
        <div
          key={idx}
          className="border border-border rounded p-1.5 flex flex-col gap-1"
        >
          <div className="flex items-center gap-1">
            <input
              ...
              className="flex-1 min-w-0 text-[10px] text-foreground bg-transparent border-b border-border outline-none px-0.5"
              placeholder="Label"
            />
            <button
              className="text-[10px] text-muted-foreground hover:text-red-400 transition-colors leading-none shrink-0"
          ...
          <div className="flex items-center gap-1">
            <span className="text-[9px] text-muted-foreground shrink-0">id:</span>
            <input
              type="text"
              value={action.name}
              onChange={(e) => updateActionField(idx, "name", e.target.value.replace(/\s/g, "_"))}
              className="flex-1 min-w-0 text-[9px] text-muted-foreground bg-transparent outline-none font-mono px-0.5"
          ...
          <div className="flex flex-wrap gap-0.5">
            {action.keys.map((key) => (
              <span
                key={key}
                className="inline-flex items-center gap-0.5 bg-card border border-border rounded px-1 py-0.5 text-[9px] text-foreground font-mono"
              >
                {key}
                <button
                  className="text-muted-foreground hover:text-red-400 transition-colors leading-none"
    ...
    case "code_block":
      return (
        <div className="flex flex-col gap-1.5">
          ...
          <div>
            <span className="text-[10px] text-muted-foreground block mb-1">Code</span>
            <textarea
              value={typeof data.code === "string" ? data.code : ""}
              onChange={(e) => onUpdateData({ code: e.target.value })}
              className="w-full bg-card border border-border rounded px-2 py-1.5 text-[11px] text-foreground font-mono outline-none resize-y min-h-20 max-h-60 transition-colors focus:border-border"
              spellCheck={false}
              rows={10}
            />
          </div>
        ...
        <div className="text-xs flex flex-col gap-2">
          ...
          {sourcePort && (
            <div className="flex items-center gap-1.5">
              ...
              />
              <span className="text-foreground">{sourcePort.dataType}</span>
            </div>
          )}
        </div>
        <Separator />
    ...
    return (
      <div className="p-3 flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <select
            className="text-xs uppercase tracking-wide text-foreground font-medium bg-card border border-border rounded px-1.5 py-0.5 outline-none cursor-pointer hover:border-border transition-colors"
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <NumberField.Group>
              <NumberField.Input className="bg-card border border-border rounded px-2 py-1 text-sm outline-none w-full transition-colors focus:border-border" />
            </NumberField.Group>
          </NumberField.Root>
          <NumberField.Root
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            ...
            <span className="text-xs text-foreground ml-1">{selectedNode.width}</span>
          </div>
          <div className="flex-1">
            <span className="text-[10px] text-muted-foreground">H</span>
            <span className="text-xs text-foreground ml-1">{selectedNode.height}</span>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-1 text-xs">
              {typeof selectedNode.data.fileName === "string" && (
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground truncate ml-2 max-w-35">{selectedNode.data.fileName}</span>
                </div>
              )}
              {typeof selectedNode.data.fileType === "string" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Type</span>
                  <span className="text-foreground">{selectedNode.data.fileType}</span>
                </div>
              )}
              {typeof selectedNode.data.fileSize === "number" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Size</span>
                  <span className="text-foreground">{formatFileSize(selectedNode.data.fileSize)}</span>
                </div>
              )}
            </div>
