...
// just 8 anchors the user drags onto real holes:
//
//   • Terminal banks (cols 0-4 and 5-9): 4 corner anchors each → bilinear
//     interpolation fills every hole in the bank.
//   • Power rails (cols -2,-1,10,11): NO anchors. The whole board is one rigid
//     grid, so each rail column rides the same warp as its neighbouring bank —
//     extrapolated past the bank edge by the rail's known schematic x-offset.
//     (Which rows carry a rail hole is the `isRailRow` block/gap pattern.)
//
...
/** Schematic (default) world x for a column. Every row shares the same x per
 *  col, so row 0 is representative. Precomputed for the cols we place — rails
 *  included — since it drives the rail extrapolation on every hole. */
const DEFAULT_COL_X: Record<number, number> = (() => {
  const map: Record<number, number> = {}
  for (let col = -2; col <= 11; col++) map[col] = worldOf(0, col).x
  return map
})()

function defaultColX(col: number): number {
  return DEFAULT_COL_X[col] ?? worldOf(0, col).x
}
...
function load(): GridCalibration {
  ...
  try {
    ...
    if (!raw) return base
    // Only height + the 8 bank corners are read; any older `rails` field is
    // ignored (rails are now derived, not anchored).
 ...
 *  the live calibration. Every column — terminals AND power rails — rides one
 *  bank's bilinear warp: the terminal columns interpolate within [0,1], the
 *  rail columns extrapolate past the bank edge by their schematic x-offset, so
 *  no separate rail anchors are needed. */
export function warpedGridXZ(row: number, col: number): { x: number; y: number; z: number } {
  ...
  const ce = useRight ? 9 : 4
  const denom = defaultColX(ce) - defaultColX(cs)
  const u = denom !== 0 ? (defaultColX(col) - defaultColX(cs)) / denom : 0
